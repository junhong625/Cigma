...
public class TeamController {
    ...
    // 팀생성
    @PostMapping()
    public CustomResponseEntity<? extends Object> createTeam(HttpServletRequest request, @RequestBody TeamCreateRequest teamCreateRequest) throws IOException {
        try {
...
//                return ResponseHandler.generateResponse(false, "존재하지 않는 사용자입니다", HttpStatus.BAD_REQUEST, null);
    ...
    // 팀명 변경
    @PatchMapping("/{id}")
    public CustomResponseEntity<? extends Object> changeTeamname(HttpServletRequest request, @PathVariable("id") Long teamIdx, @RequestBody TeamPatchRequest teamPatchRequest) throws IOException {
        try {
            UserPrincipal userPrincipal = SecurityUtils.getUserPrincipal();
            Optional<User> user = userService.findById(userPrincipal.getUserIdx());
    ...
    public CustomResponseEntity<? extends Object> changeMembers(HttpServletRequest request, @PathVariable("id") Long teamIdx, @RequestBody TeamMembersRequest teamMembersRequest) throws IOException {
        try {
            UserPrincipal userPrincipal = SecurityUtils.getUserPrincipal();
            Optional<User> user = userService.findById(userPrincipal.getUserIdx());
